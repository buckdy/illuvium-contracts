...
const { generate_tree } = require("../../test/protocol/include/merkle_tree_utils");
...
function load_sale_data_csv(file_path = "./sale_data.csv") {
  ...
  return csv_rows
    ...
    .map(row =>
      row
        .split(",") // convert comma separated text into array (metadata)
        .map(s => parseInt(s)),
    ) // of integers (numbers)
    .map(metadata =>
      Object.assign(
