// Chainlink Illuvitars Price Oracle Tests
...
function get_interface_id(contract_interface) {
  let interface_id = ethers.constants.Zero;
  const functions = Object.keys(contract_interface.functions);
  for (let i = 0; i < functions.length; i++) {
    interface_id = interface_id.xor(contract_interface.getSighash(functions[i]));
  }

  return interface_id;
}

// run illuvitars tests
contract("IlluvitarsPriceOracleV1: Chainlink Feed Tests", function (accounts) {
  ...
  describe("deployment / proxy initialization", function () {
    ...
    it("fails if Chainlink aggregator address is not set", async function () {
      await expectRevert(illuvitars_price_oracle_deploy_pure(a0, ZERO_ADDRESS), "aggregator address is not set");
    });
    describe("fails if Chainlink aggregator returns invalid data", function () {
      it("fails if Chainlink aggregator returns invalid roundId", async function () {
        ...
        await expectRevert(
          illuvitars_price_oracle_deploy_pure(a0, aggregator.address),
  ...
  describe("when oracle is deployed", function () {
    ...
    it("Supports ERC165 interface check", async function () {
      const interface = new ethers.utils.Interface(artifacts.require("./IlluvitarsPriceOracle").abi);
      expect(
        await oracle.supportsInterface(ethers.BigNumber.from(get_interface_id(interface)).toHexString()),
      ).to.be.true;
    });
