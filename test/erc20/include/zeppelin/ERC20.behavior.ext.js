...
function shouldHaveMint(errorPrefix, initialSupply, initialHolder, recipient) {
  describe("_mint", function () {
    ...
    it("rejects a null account", async function () {
      await expectRevert(this.token.mint(ZERO_ADDRESS, amount), `${errorPrefix}mint to the zero address`);
    });
...
function shouldHaveBurn(errorPrefix, initialSupply, initialHolder) {
  describe("_burn", function () {
    it("rejects a null account", async function () {
      await expectRevert(this.token.burn(ZERO_ADDRESS, new BN(1)), `${errorPrefix}burn from the zero address`);
    });
